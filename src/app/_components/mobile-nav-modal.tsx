    ...
    <AnimatePresence>
      {modals.includes('mobile-nav') ? (
        <motion.div
          ...
          <button
            onClick={() => remove()}
            className="absolute top-8 right-8 flex items-center justify-center border border-zinc-900 p-2 rounded-full  h-12 w-12 flex active:bg-zinc-50">
            <ExitIcon />
          </button>
          <div className="flex flex-col mb-12 h-1/2 w-full items-center">
            {NAV_SECTIONS.map(({ title, path }, i) => {
              return (
                <Link key={path} href={path} onClick={() => remove()} className={`${i != 0 ? 'mt-8' : ''} text-6xl w-full  text-center active:bg-zinc-50`}>{title}</Link>
              )
            })}
          </div>
        </motion.div>) : null
