            ...
            {images.map(img => {
              return (
                <Link href={`film/${img}`} className="relative h-44 w-44">
                  <Image key={img} alt={`${img} film`} className="object-cover lg:rounded hover:object-right-bottom transition-all duration-[1000ms] ease-in-out" sizes="(min-width: 300px) 50vw, 100vw" src={`/assets/film/${img}`} fill />
                </Link>
              )
            })}
          </div>
